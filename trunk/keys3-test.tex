...
\SetModuleKeys{
  key/.create_choices:nn = {a,b,c}
    {Choice `\csname l_keys_current_choice_tlp\endcsname' is number
...
% #1#2->#1 or #2.
%
% > \keys-root/module name/key one/._num_args_tlp=macro:
% ->2.
% ----------------------------------------------------------------------
\SetModuleKeys{
  ...
  key one = ab,
  key one/.show_code:,
  key one/._num_args_tlp/.show_key:,
}
